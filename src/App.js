...
const App = props => {
        ...
        let [isSend, changeIsSend] = useState(false);

        let [isAnswers, changeIsAnswers] = useState(false);

        const [answersArr, changeAnswersArr] = useState({});
        ...
        const updateForms = (id, newData) => {
            const form = {...props.forms.find(item => item.id === id), ...newData};
        ...
        <>
            <div className="wrapper" style={appStyle.wrapper}>
                <header style={appStyle.header}>
                    <h1>Опросы online</h1>
                    {isAnswers ? <button> <a href="/user" style={appStyle.homeButton}>Личный кабинет</a></button> : null}
                </header>
                <Router>
                        <Route exact path={'/'}>
                            { !props.isLogin ? <Authorize login={props.login}/> : <Redirect to={'/user'}/>}
                        </Route>
                        <Route exact path={'/restore'}>
                            { !props.isLogin ? <RestorePassword/> : <Redirect to={'/user'}/>}
                        </Route>
                        <Route path={'/user'}>
                            <>
                            { isAnswers ? <Redirect to={'/answers'}/> : null}
                                <ListForms 
                        ...
                        <Route path={'/form/:id'}>
                            ...
                            : <div style={appStyle.info}><span>Ответы отправлены!</span></div>}
                            </>
                        </Route> 
                        <Route path={'/answers'}>
                            <>
                                { isAnswers ? <FormAnswers answers={props.answers}/> : <div style={appStyle.info}>
                                    <span>Для просмотра ответов перейдите в </span> <button> <a href="/user" style={appStyle.homeButton}>Личный кабинет</a></button>
                                    </div>}
                            </>
                        </Route>                                 
                </Router>
