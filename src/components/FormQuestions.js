...
const FormQuestions = props => {
  ...
  const [formData,
    changeInputValue] = useState({});

  const [isFetch,
    changeIsFetch] = useState(false);

  const [formAnswer,
    changeFormAnswer] = useState({});

  const [allAnswers,
    changeAllAnswers] = useState({});
  ...
  return (
    ...
    <div
      ...
      <div className="lists-item-header" style={{}}>
        <h2 style={{
          textAlign: 'center'
        }}>{formData.name ?? "Загрузка опроса..."}</h2>
        <div style={{
          margin: '40px 0'
        }}>
        ...
        ? (
          <form
            ...
            {formData
              .questions
              .map((question, index) => {
                return ( <> <label style={formEditStyle.label} htmlFor={`answer-${ (index + 1)}`}>
            ...
            <label style={formEditStyle.label} htmlFor={'answerId'}>
              ...
              <input
                ...
                type="text"
                name={'answerId'}
                placeholder="Напишите свой id для отправки формы"
                required/>
