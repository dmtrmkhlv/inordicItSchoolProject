...
const FormAnswers = props => {
    ...
    return (   
        <div key={nanoid()} className="questions-box" style={formEditStyle.formBox}>
            <div className={`lists-item`} style={{width: '100%', padding: '20px'}}>
                <div className="lists-item-header" style={{}}>
                    <h2 style={{textAlign: 'center'}}>{props.answers.length>0 ? `Ответы на опрос: ${props.answers[0].name}` : "Ответов еще нет"}</h2>
                ...
                {  props.answers?.map((answer) => {
                    return (
                        <div key={nanoid()} style={formEditStyle.answerCard} className="answer-card">
                            ...
                            {Object.keys(answer.allAnswers)?.map(i => (
                                    <div key={nanoid()} style={formEditStyle.answerCardBlock} className="answer-card-block">
